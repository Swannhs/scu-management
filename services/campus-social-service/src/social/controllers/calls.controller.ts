...
export class CallsController {
  ...
  async endRoom(
    ...
    const userId = req.user?.sub as string;
    return this.callsService.endRoom(tenantContext.effectiveTenantId, userId, roomId);
  }

  @Get('calls/rooms/:roomId/participants')
  @Roles({ roles: ['STUDENT', 'FACULTY', 'TENANT_ADMIN'] })
  async listParticipants(@TenantContextParam() tenantContext: TenantContext, @Param('roomId') roomId: string) {
    return this.callsService.listParticipants(tenantContext.effectiveTenantId, roomId);
  }
