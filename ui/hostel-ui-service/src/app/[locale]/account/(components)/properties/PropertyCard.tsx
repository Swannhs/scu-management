    ...
    return (
        ...
        >
            ...
            <Link href={`/property/${data?.id}`}>
                <PropertyCardContent
                    estateAdditionalPrice={String(data?.estateAdditionalPrice ?? "")}
                    rooms={data?.rooms ?? 0}
                    title={data?.title ?? ""}
                    estatePriceType={data?.estatePriceType ?? ""}
                    estatePrice={String(data?.estatePrice ?? "")}
                    livingArea={String(data?.livingArea ?? "")}
